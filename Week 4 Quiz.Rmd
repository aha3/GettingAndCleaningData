...
gdpdateDownloaded <- date()

# need to specify how many lines to skip and rows to read

gdpData2 <- read.csv("gdp.csv", header = FALSE, skip = 10, nrows = 190)
```

Remove commas from GDP variable (V5) using gsub(), and compute mean mean.

```{r}
gdpDataClean <- as.numeric(gsub(",", "", gdpData$V5))

answer2 = mean(gdpData$V5, na.rm = TRUE)

print(answer2)
```

I keep getting 190711...going to try somebody else's code

```{r}
download.file("https://d396qusza40orc.cloudfront.net/getdata%2Fdata%2FGDP.csv", destfile = "quiz4data.csv")

data <- read.csv("quiz4data.csv") # this reads in more than 190 rows (330 observations)

clean <- gsub(",","",data[,5]) # this cleans out ALL rows from the 5th column, but remains a character vector

mean(as.numeric(clean[1:215]),na.rm = TRUE)
```

trying again...with a combination of my code and the chunk above that worked
